...
# English dataset
class BenchmarkDataset(PromptRawDataset):
    ...
    def get_train_data(self):
        ...
        if 'dolphin' in self.dataset_name_clean or 'OpenOrca' in self.dataset_name_clean:
            return random.sample(list(self.raw_datasets["train"]), int(len(self.raw_datasets["train"])*0.01))
        return self.raw_datasets["train"]
    ...
    def get_prompt(self, sample):
        if 'dolphin' in self.dataset_name_clean or 'Open-Platypus' in self.dataset_name_clean:
            return sample['instruction'] + sample['input']
        elif 'OpenOrca' in self.dataset_name_clean:
            return sample['question']

    def get_chosen(self, sample):
        if 'dolphin' in self.dataset_name_clean:
            return sample['output']
        elif 'Open-Platypus' in self.dataset_name_clean:
            return sample['output']
        elif 'OpenOrca' in self.dataset_name_clean:
...
# Custom dataset
def get_llama2_prompt(input_sample): 
    if 'system_prompt' in input_sample:
            system_prompt = input_sample['system_prompt']
    else:
        system_prompt = \
'''
...
'''
    instruct = input_sample['instruct'] + '\n\n'

    inp = ''
    if 'input' in input_sample and input_sample['input'] and input_sample['input'].strip() != '':
        inp = '### Input:\n' + input_sample['input']
    resp = ''
    prompt = instruct + inp + resp
...
{prompt}[/INST] '''
    prompt = prompt_format.format(system_prompt=system_prompt, prompt=prompt)
    return prompt
...
class CustomJsonDataset():
    ...
    def get_prompt(self, sample):
        return get_llama2_prompt(sample)
    ...
    def get_prompt_and_chosen(self, sample):
        return get_llama2_prompt(sample) + sample['output']
...
class kaggleTWDataset(object):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.dataset_name = dataset_name
        self.dataset_name_clean = dataset_name.replace("/", "_")
        print('succeed loading kaggle-tw')
    ...
    def get_prompt_and_chosen(self, sample):
        if sample['prompt'] is not None and sample['chosen'] is not None:
            return sample['prompt'] + ' ANS:' + sample['chosen']
        return None

    def get_prompt_and_rejected(self, sample):
        if sample['prompt'] is not None and sample['rejected'] is not None:
            return sample['prompt'] + ' ANS:' + sample['rejected']
        return None
