...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif 'dolphin' in dataset_name or 'OpenOrca' in dataset_name or 'Open-Platypus' in dataset_name:
        return raw_datasets.BenchmarkDataset(output_path, seed, local_rank,
                                             dataset_name)
    elif "gsm8k" in dataset_name:
        print("load gsm8k dataset")
        return raw_datasets.BenchmarkGsm8k(output_path, seed, local_rank,
                                             dataset_name)
    elif "doxy" in dataset_name:
        print("load doxy dataset")
        return raw_datasets.DemoDoxygen(output_path, seed, local_rank,
                                             dataset_name)
    elif "liaw" in dataset_name:
        print("load liaw dataset")
        return raw_datasets.LiawDataset(output_path, seed, local_rank,
                                             dataset_name)
    elif "kaggle-tw" in dataset_name:
        print("load kaggle-tw dataset")
        return raw_datasets.kaggleTWDataset(output_path, seed, local_rank,
                                             dataset_name)
    elif "pretrain" in dataset_name:
        print("load pretrain dataset")
        return raw_datasets.PretrainDataset(output_path, seed, local_rank,
                                             dataset_name)
    elif "alpaca_2k" in dataset_name:
        print("load alpaca_2k dataset")
        return raw_datasets.AlpacaDataset(output_path, seed, local_rank,
                                             dataset_name)
    else:
        print("load custom dataset")
        return raw_datasets.CustomJsonDataset(output_path, seed, local_rank,
                                             dataset_name)
...
class IterablePromptDataset(Dataset):
    def __init__(self, 
    current_dataset, 
    raw_dataset,
                 pad_token_id:int, 
        ...
        super().__init__()
        self.dataset_size = 0
        self.chosen_dataset, self.reject_dataset, self.prompt_dataset = self.__checker__(current_dataset, raw_dataset, train_phase, end_of_conversation_token)
    ...
    def __checker__(self, current_dataset, raw_dataset, train_phase, end_of_conversation_token):
        ...
        if train_phase == 1:
            chosen_dataset = []
            for index, tmp_data in enumerate(current_dataset):
                # tokenize the text
        ...
        return chosen_dataset, reject_dataset, prompt_dataset
    ...
    def __getitem__(self, idx):
        ...
        if self.train_phase == 1:
            chosen_token = self.chosen_dataset[idx]
            chosen_token = self.tokenizer(chosen_token,
            ...
            chosen_token = self.chosen_dataset[idx]
            reject_token = self.reject_dataset[idx]
            ...
            reject_token = self.tokenizer(reject_token,
                                         max_length=self.max_seq_len,
                                         padding="max_length",
                                         truncation=True,
                                         return_tensors="pt")
            
            return chosen_token[idx]["input_ids"], chosen_token[idx]["attention_mask"], \
                reject_token[idx]["input_ids"], reject_token[idx]["attention_mask"]
        
        elif self.train_phase == 3:
            prompt_token = self.tokenizer(self.prompt_dataset[idx], return_tensors="pt")
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    """
    return IterablePromptDataset(current_dataset, raw_dataset,
                         tokenizer.pad_token_id, train_phase, tokenizer, end_of_conversation_token, max_seq_len) #############################################
