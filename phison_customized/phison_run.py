...
from utils.utils import create_swap_data_path
import json 
...
def parse_args():
    ...
    parser.add_argument(
         ...
         type=int,
         default=-1,
         help="max iteration for each epoch")
    
    parser.add_argument(
         "--allingpu",
         type=int,
         default=0,
         help="allingpu mode, setting 1 to disable aiDAPTIV")
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help="(lora) Path to pretrained Lora weight.")
    
    parser.add_argument(
         "--target_modules",
         type=list,
         default=['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj', 'embed_tokens', 'lm_heed'],
         help="(lora) target modules to apply low rank adaption")
    
    parser.add_argument(
         "--task_type",
         type=str,
         default='CAUSAL_LM',
         help="(lora) training task type")

    parser.add_argument(
         "--rank",
         type=int,
         default=8,
         help="(lora) dimension of the low-rank matrices")
    
    parser.add_argument(
         "--lora_alpha",
         type=int,
         default=16,
         help="(lora) scaling factor for the weight matrices")
    ...
    parser.add_argument(
        "--save_precomputed_logits_dataset_path",
        type=str,
        default=None,
        help="Path to save precomputed logits dataset path for dpo dataset.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="temperature hyperparameter of DPO, ypically in the range 0.1 to 0.5. This controls how much we pay attention to the reference model in the sense that as beta gets smaller the more we ignore the reference model.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sigmoid",
        choices=["sigmoid", "hinge", "ipo", "kto_pair", "bco_pair"],
        help="loss function type for DPO",
    )
...
if __name__ == "__main__":
    ...
    # swap_data_path = create_swap_data_path(args.nvme_path, args.local_rank)
    swap_data_path = create_swap_data_path(args.nvme_path, args.local_rank)
    args.swap_path = swap_data_path
    param['nvme_path'] = swap_data_path
    ...
    if MODEL_RESOURCES_CONFIG['prepare_function'] is not None:
        train_dataloader, data_collator = MODEL_RESOURCES_CONFIG['prepare_function'](args)
    else:
        raise ValueError(f'config file must contain prepare function, please check the config file')
    
    # DPO
    if MODEL_RESOURCES_CONFIG['dataset'] is not None:
        dataset = MODEL_RESOURCES_CONFIG['dataset']
    else:
        dataset = None
    ...
    try:
        trainer.train(args, task_name, train_dataloader, data_collator, tokenizer, dataset)
    except Exception as e: 
        if os.path.exists(swap_data_path) and global_rank == 0:
            subprocess.run(['rm', '-r', swap_data_path])

        print_rank_0(f"[FAIL] An error occurred: {e}", global_rank)
        raise e
    finally:
        print_rank_0(f'[INFO] Exist process, remove swap data path', global_rank)
        if os.path.exists(swap_data_path) and global_rank == 0:
            subprocess.run(['rm', '-r', swap_data_path])
